...
export default function ChangeMobileNumberScreen({ navigation }: any) {
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      if (waitSeconds) {
        // Rate-limited: stay on ENTER, don't pretend a fresh OTP was sent
          ...
          {step === 'ENTER' && (
            <>
              ...
              <TextInput
                ...
                value={newMobile}
                onChangeText={t =>
                  setNewMobile(t.replace(/\D/g, '').slice(0, 10))
                }
              ...
              <TouchableOpacity
                style={[styles.primaryBtn, (!newMobile || loading || resendIn > 0) && styles.primaryBtnDisabled]}
                onPress={sendOtp}
                disabled={!newMobile || loading || resendIn > 0}
              >
                {loading ? (
