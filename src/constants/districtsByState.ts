// Static state -> district mapping for India. Keys must match the state
// names in `indianStates.ts` exactly, so the District/City filter can be
// derived from whichever state(s) the user picks.
export const DISTRICTS_BY_STATE: Record<string, string[]> = {
  ...
  Telangana: [
    ...
    'Siddipet', 'Suryapet', 'Vikarabad', 'Wanaparthy', 'Warangal', 'Hanumakonda', 'Yadadri Bhuvanagiri',
  ],
...
export const getDistrictOptionsForStates = (states: string[] = []) => {
  const districts = new Set<string>();
  states.forEach((state) => {
    (DISTRICTS_BY_STATE[state] || []).forEach((district) => districts.add(district));
  });
